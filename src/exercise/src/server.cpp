...
public:
    ExeDistanceServer():Node("exe_distance_server"),turtle1_x(0.0),turtle1_y(0.0){
        ...
        // 3-2.创建服务端；
        distance_server = this->create_service<base_interface::srv::Distance>("distance",std::bind(&ExeDistanceServer::distanceCallBack, this, std::placeholders::_1, std::placeholders::_2));
    }
private:
    ...
    // 3-3.解析目标值，计算距离并反馈结果。
    void distanceCallBack(const base_interface::srv::Distance_Request::SharedPtr request,
                    base_interface::srv::Distance_Response::SharedPtr response
    ){
        // 解析目标值
    ...
    rclcpp::Service<base_interface::srv::Distance>::SharedPtr distance_server;
    float turtle1_x;
    float turtle1_y;
