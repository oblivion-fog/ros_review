...
public:
    ExeDistanceClient():Node("exe_distance_client"){
        // 3-1.创建客户端；
        distance_client = this->create_client<base_interface::srv::Distance>("distance");
    }   
    // 3-2.连接服务；
    bool connect_server(){
    ...
    // 3-3.发送请求。
    rclcpp::Client<base_interface::srv::Distance>::FutureAndRequestId send_distance(float x,float y,float theta){
        auto distance_request = std::make_shared<base_interface::srv::Distance::Request>();
...
private:
    rclcpp::Client<base_interface::srv::Distance>::SharedPtr distance_client;
};
